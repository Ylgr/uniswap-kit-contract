...
contract TestBicOracleFromEth2 is Ownable {
    address public baseToken;
    address public quoteToken;
    address public pool;
    uint32 public period;
    uint256 private _cachePrice;
    uint256 private _cacheCounter;
    ...
    function getTokenValueOfEth(uint256 ethOutput) external returns (uint256 tokenInput) {
        if(_cacheCounter == 0) {
            _cachePrice = getQuoteAmount(1e8);
            _cacheCounter = 1;
        } else {
            _cacheCounter = 0;
        }
        uint256 quoteAmount = _cachePrice;
